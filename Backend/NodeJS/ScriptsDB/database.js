...
con.connect(function(err) {
	...
	sql = "create table if not exists interessadosObjeto(interessado int not null,objeto int not null, tipo varchar(255) ,primary key(interessado,objeto),constraint iO1 foreign key (interessado) references interessados(idInteressado),constraint iO2 foreign key (objeto) references objetos(idObjeto)    )ENGINE = InnoDB;";
	con.query(sql, function (err, result) {
		if (err) throw err;
	...
	sql = "create table if not exists iluminacao(idIluminacao int UNIQUE not null AUTO_INCREMENT,fonte varchar(255),iluminancia varchar(50),UVmedido varchar(50),UVreal varchar(50),tipo set('artificial','natural'),objeto int,primary key(idIluminacao),constraint il1 foreign key(objeto) references objetos(idObjeto))ENGINE = InnoDB;";
	con.query(sql, function (err, result) {
		if (err) throw err;
		console.log("Table iluminacao created");
	});
	//criar tabela poluicao
	sql = "create table if not exists poluicao(idPoluicao int UNIQUE not null AUTO_INCREMENT,agente TEXT,fonte varchar(255),resultados TEXT,objeto int not null,primary key(idPoluicao),constraint pl1 foreign key(objeto) references objetos(idObjeto))ENGINE = InnoDB;";
	con.query(sql, function (err, result) {
		if (err) throw err;
	...
	//criar tabela testes
	sql = "create table if not exists testes(idTeste int UNIQUE not null AUTO_INCREMENT,referencia varchar(255),localizacao varchar(255),objetivos varchar(255),resultados varchar(255),data date,objeto int not null,tecnico int not null,primary key(idTeste), constraint te1 foreign key(objeto) references objetos(idObjeto),constraint te2 foreign key(tecnico) references tecnicos(idTecnico))ENGINE = InnoDB;";
	con.query(sql, function (err, result) {
		if (err) throw err;
...
//-------------------------------LINHA DE AGUA---------------------------------------------------------------
	...
	sql = "INSERT INTO interessados(idInteressado, nome, NIF, enderecoPostal,enderecoEletronico,contacto) values (1, 'Zuao', '123454678', 'Coimbra','Sem informação','Sem informação'),(2, 'Salvador Sanchez', '111222333', 'Lisboa','Sem informação','Sem informação'),(3, 'Ze', '444555666', 'Não se aplica','Não se aplica','Não se aplica');";
	con.query(sql, function (err, result) {
		if (err) throw err;
		console.log("Values inserted");
	});	
	//inserir registos dos interessadosObjeto
	sql = "INSERT INTO interessadosObjeto(interessado,objeto,tipo) VALUES (1,1,'Proprietário'),(2,1,'Dono da obra'),(3,1,'Mecenas');";
	con.query(sql, function (err, result) {
		if (err) throw err;
		console.log("Values inserted");
	});
	...
	sql = "INSERT INTO testes(idTeste,referencia,localizacao,objetivos,resultados,data,objeto,tecnico) VALUES (1,'Observação á vista desarmada','Corpo da coluna e capitel','Identificação dos materiais','Confirmação do uso de folha de ouro, purpurina, massa oleica, camada de preparação de gesso ou cré, policromia.','2017-02-21',1,1);";
	con.query(sql, function (err, result) {
		if (err) throw err;
		console.log("Values inserted");
	});
